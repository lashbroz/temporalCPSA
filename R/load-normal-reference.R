...
#'
#' @details
#' The package copy stores the normal/reference matrices, sample metadata, and
#' when available, the manuscript protein N-TMP consensus cluster assignments
#' needed to reproduce the manuscript age-dependent trajectory analyses without
#' loading the original private `for_tadj.RData` object.
#'
#' For protein Figure 2 style heatmaps, the stored `protein$clusters$consensus_k4`
#' vector follows the manuscript remapping of the original normal consensus
#' classes: raw cluster 3 is displayed as N-TMP 1, raw cluster 4 as N-TMP 2,
#' raw cluster 1 as N-TMP 3, and raw cluster 2 as N-TMP 4. These labels are
#' part of the heatmap ordering logic, not just cosmetic colors.
#'
#' In the manuscript protein trajectory workflow, these normal/reference data
#' are adjusted with `score ~ pH + PMI + Ethnicity`. Missing pH is imputed from
#' the full normal-reference metadata before sex stratification, ethnicity label
#' `H` is combined with `C`, and `C` is used as the reference level. These
#' details are implemented in [ageTMP_compare_normal_tumor_trajectory()] for
#' reproducibility.
#'
...
ageTMP_load_normal_reference <- function(path = NULL) {
  if (is.null(path)) {
    path <- system.file("extdata", "normal_reference.rds", package = "temporalCPSA")
  }
